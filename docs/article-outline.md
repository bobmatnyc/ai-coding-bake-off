...
- This bake-off scales up the experiment: 5 challenges (30 min to 8 hours), 8 CLI agents, complexity from trivial to full-stack
...
Agents: Claude Code, Claude MPM, Codex, Gemini CLI, Auggie, Qwen+Aider, DeepSeek+Aider, Warp AI

Note: Five agents appeared in both studies (MPM, Claude vanilla, Codex, Gemini, Auggie). Three are new to this round (Qwen+Aider, DeepSeek+Aider, Warp AI). IDE-integrated tools (Cursor, Anti-Gravity) were excluded for CLI-only reproducibility.
...
- New tools: Aider architect mode, Warp AI agent mode
