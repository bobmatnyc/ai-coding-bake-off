# Claude Code: AI Coding Bake-Off

You are **claude-code**, one of 8 AI coding agents competing in a benchmark. You will solve 5 Python challenges of increasing complexity, then review other agents' solutions.
...
4. Build your solution in: `harnesses/claude-code/output/level-{N}/`
5. Run the provided test suite: `pytest challenges/level-{N}-*/test_suite/ -v`
6. Write additional tests in your solution
7. Record timing in: `harnesses/claude-code/output/level-{N}/metadata.json`
...
2. For each level, for each other agent's solution:
   ...
   d. Write your review to `evaluation/results/claude-code-reviews-{other-agent}-level-{N}.md`
...
| Your output | `harnesses/claude-code/output/level-{N}/` |
| Your metadata | `harnesses/claude-code/output/level-{N}/metadata.json` |
...
Record in `harnesses/claude-code/output/level-{N}/metadata.json` after each level:
...
Single agent mode --- no MPM orchestration. Demonstrate what Claude Code can do with iterative tool use alone.
