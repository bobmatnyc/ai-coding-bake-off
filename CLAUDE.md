...
|---------|-------------|
| `solve level 1` | Read prompt -> build solution in harnesses/claude-mpm/output/level-1/ -> run tests -> record metadata |
...
| `evaluate level 1` | Run automated scoring + blind cross-review of all harnesses' level-1 solutions |
| `evaluate all` | Full evaluation across all levels and harnesses, generate comparison report |
...
- Each harness's output is isolated: `harnesses/{agent-name}/output/level-{N}/`
- Solutions must be self-contained Python projects within their output directory
- **Do NOT look at other harnesses' solutions** until cross-review phase
- All solutions target Python 3.12+
- Record timing/token metadata in `metadata.json` per solution
- When run under Claude MPM, the agent name is `claude-mpm` --- all solutions go in `harnesses/claude-mpm/output/level-{N}/`
...
├── harnesses/           # Agent workspaces (instructions + output)
│   ├── claude-mpm/      # Claude MPM harness
│   │   ├── instructions/  # CLAUDE.md, .claude-mpm/, setup.md
│   │   └── output/        # Solutions go here
│   ├── claude-code/     # Claude Code harness
│   ├── cursor/          # Cursor harness
│   ├── codex/           # OpenAI Codex harness
│   ├── gemini/          # Gemini harness
│   ├── anti-gravity/    # Anti-Gravity harness
│   ├── augment/         # Augment Code harness
│   ├── qwen-aider/      # Qwen 2.5 + Aider harness
│   └── deepseek-aider/  # DeepSeek + Aider harness
...
3. Build solution in: `harnesses/claude-mpm/output/level-{N}/`
4. Run provided tests: `pytest challenges/level-{N}-*/test_suite/ -v`
5. Write additional tests in your solution
6. Record metadata in `harnesses/claude-mpm/output/level-{N}/metadata.json`
